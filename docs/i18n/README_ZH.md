...
|------|------|------|
| Node.js | >= 22 | 必需 |
| pnpm | 最新版 | 必需 — `npm install -g pnpm` |
...
cd ScienceClaw
bash scripts/setup.sh       # 自动安装依赖、配置 API 密钥和渠道
```
...
```bash
./scienceclaw add telegram   # 或 discord, slack, whatsapp, feishu, matrix, wechat
./scienceclaw channels       # 查看已配置的渠道
```
