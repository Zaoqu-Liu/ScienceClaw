...
| 도메인 | 수 | 스킬 |
|--------|-----|------|
| **생물정보학** | 30+ | `scanpy`, `anndata`, `deseq2`, `clusterProfiler`, `GSEA`, `arboreto`, `pyDESeq2` |
| **시각화** | 35+ | `volcano-plot`, `heatmap`, `km-survival-curve`, `forest-plot`, `circos-plot`, `manhattan-plot` |
| **약물 발견** | 20+ | `chembl-database`, `rdkit`, `zinc-database`, `alphafold-database`, `adaptyv`, `medchem` |
| **임상 및 생존** | 15+ | `clinicaltrials-database`, `meta-analysis`, `survival-analysis`, `mendelian-randomization` |
| **단일 세포** | 10+ | `scanpy`, `scvi-tools`, `cellxgene-census`, `anndata`, `cell-type-annotation` |
| **유전체학** | 15+ | `gene-database`, `ensembl-database`, `gwas-catalog`, `clinvar`, `variant-annotation` |
| **데이터베이스** | 20+ | `uniprot-database`, `pdb-database`, `string-database`, `opentargets`, `reactome` |
| **머신러닝** | 10+ | `scikit-learn`, `xgboost`, `feature-selection`, `shap-interpretation`, `aeon` |
